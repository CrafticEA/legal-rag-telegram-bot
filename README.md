...
Проект для юридического Q&A по загруженным документам (RAG): загрузка файлов, индексация, поиск релевантных фрагментов и генерация ответов локальной LLM. Telegram-бот реализован через **n8n** (webhook + Telegram Trigger), а backend состоит из **Rails API** + двух **FastAPI** сервисов (`rag`, `llm`).
